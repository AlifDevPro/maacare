...
export function deriveOnboardingFocus(d: SignupProfileDraft): {
  ...
  return {
    ...
    modelInstruction:
      `Name and role are already saved ("${d.displayName.trim()}", ${professionLabel(d.profession)}). Do not ask for name or role again unless the user corrects them. If the latest user message changes pregnancy intent (e.g. not pregnant) or role (e.g. student_researcher), you MUST update pregnancyStatus and/or profession in DRAFT_PATCH. Otherwise offer one short optional question (dates, journey detail, health note) or remind them they can use the secure email/password section below. Keep it brief.`,
  };
}
