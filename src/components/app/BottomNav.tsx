...
import { APP_SHELL_COLUMN_MAX, APP_SHELL_CONTENT_PADDING } from "@/lib/app-shell-layout";
...
export function BottomNav({ className }: { className?: string }) {
  ...
  return (
    <nav
      ...
      className={cn(
        "fixed bottom-0 left-1/2 z-40 w-full -translate-x-1/2 border-t border-border/60 bg-background/85 backdrop-blur-xl",
        APP_SHELL_COLUMN_MAX,
        className,
      )}
      style={{ paddingBottom: "env(safe-area-inset-bottom)" }}
    >
      <ul className={cn("flex items-stretch justify-between py-1.5", APP_SHELL_CONTENT_PADDING)}>
