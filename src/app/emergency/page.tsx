...
/** When offline / denied GPS: split demo rows into buckets (max 8 each) by inferred kind. */
...
export default function EmergencyPage() {
  ...
  useEffect(() => {
    ...
    async function fetchNearby() {
      setLoading(true);
      setStatusText("Loading clinics, hospitals, and pharmacies (up to 8 each)...");
      ...
      try {
        const settled = await Promise.all(
          kinds.map((category) =>
            fetchCategory(category).catch(() => [] as Hospital[]),
          ),
        );
        if (!active) return;
        ...
        if (total === 0) {
          setStatusText("No nearby rows returned. Using backup list.");
          setByKind(splitFallbackByKind());
        } else {
          setByKind(next);
          setStatusText(
            `Clinic ${settled[0].length} · Hospital ${settled[1].length} · Pharmacy ${settled[2].length} (closest first in each tab, max 8)`,
          );
        }
      } catch {
        if (!active) return;
        setStatusText("Using backup hospitals. Live lookup temporarily unavailable.");
        setByKind(splitFallbackByKind());
      } finally {
  ...
  return (
    <AppShell>
      ...
      <div className="space-y-5 px-4 pt-4">
        ...
        {/* Interactive map — pins match selected facility type */}
