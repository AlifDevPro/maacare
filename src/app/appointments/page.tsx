...
export default function AppointmentsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/appointments?status=scheduled&limit=50", { credentials: "include" });
      const j = (await res.json().catch(() => ({}))) as { appointments?: Appointment[]; message?: string };
      if (!res.ok) throw new Error(j.message ?? "Could not load appointments");
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const j = (await res.json().catch(() => ({}))) as { appointment?: Appointment; message?: string };
