  ...
  try {
    ...
    for (let i = 0; i < parsed.data.orderedIds.length; i += 1) {
      ...
      const { error } = await svc
        ...
        .update({ display_order: (i + 1) * 10, updated_by: gate.userId })
        .eq("id", id);
      if (error) return failJson(500, error.message || "Could not reorder team members.");
    }
    revalidateDocsRuntimeCaches();
    return Response.json({ ok: true });
  } catch (e) {
    return serverErrorJson("admin/docs/team/reorder POST", e);
  }
}
