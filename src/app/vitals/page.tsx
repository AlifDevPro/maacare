...
export default function VitalsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/vitals?limit=40", { credentials: "include" });
      const j = (await res.json().catch(() => ({}))) as { vitals?: Vital[]; message?: string };
      if (!res.ok) throw new Error(j.message ?? "Could not load vitals");
      setItems(j.vitals ?? []);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Could not load vitals");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const j = (await res.json().catch(() => ({}))) as { vital?: Vital; message?: string };
